...
function buildOutputStyleRules(outputMode: "json" | "ndjson", inputType: "free_text" | "choice_selected" | "auto_continue" | "continue_without_user_speech") {
  ...
  return [
    ...
    "- ナレーションは環境・状況・雰囲気・心理描写。1〜3文。セリフを入れるな。連続で複数のnarration itemを並べてよい（場面転換、スマホ通知、引用文『...』など）。",
...
function buildResponseFormatPrompt(
  ...
  return [
    ...
    "- smartReplies: ユーザーキャラの返信案。行動描写＋セリフを混ぜたフルテキスト形式。2〜3件。例:",
    "  - 「なるほど、と呟いて合点がいった様子で頷く あー、あれか...ニュースでやってたやつ」",
    "  - 「頭を抱えて溜息をつく ...マジかよ。そんな大事な役目、なんで俺が......」",
    "  - 「しばらくぼーっとしていてからメッセージを返信する」",
    "  - 各返信案は「行動描写（三人称）＋セリフ（一人称）」の形。行動のみもOK。セリフのみもOK。",
    "  - ユーザーキャラの性格・口調に合わせた自然な文体で書く。",
    sharedChoices,
